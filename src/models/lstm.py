...
class LSTM(torch.nn.Module):
    def __init__(self, input_size, num_layers, hidden1, hidden2,
        ...
        self.num_layers = num_layers  # Number of recurrent layers. E.g., setting num_layers=2 would mean stacking
        # two LSTMs together to form a stacked LSTM, with the second LSTM taking in outputs of the first LSTM and
        ...
        self.lstm = torch.nn.LSTM(
            ...
            dropout=dropout_prob if num_layers > 1 else 0,  # Only applies if num_layers > 1,
            batch_first=True,  # the input and output tensors are provided as (batch, seq, feature) instead
            # of (seq, batch, feature). Note that this does not apply to hidden or cell states.
        )
...
class LSTM_UQ(torch.nn.Module):
    """
    LSTM with uncertainty quantification.
    """

    def __init__(self, input_size, num_layers, hidden1, hidden2, device):
        super().__init__()
        self.input_size = input_size  # number of expected features in the input x
        self.hidden1 = hidden1  # number of features in the hidden state h
        self.hidden2 = hidden2  # number of features in feed forward neural network
        ...
        self.fc1 = torch.nn.Linear(in_features=self.hidden1, out_features=self.hidden2)
        self.fc2 = torch.nn.Linear(in_features=self.hidden2, out_features=2)
